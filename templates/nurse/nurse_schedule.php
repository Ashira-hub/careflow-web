<?php
 ...
 try {
   ...
   $sql = "SELECT id, nurse, nurse_id, nurse_email,
                  to_char(\"date\", 'YYYY-MM-DD') AS date,
                  shift, ward, notes, lower(status) AS status,
                  to_char(start_time, 'HH24:MI') AS start_time,
                  to_char(end_time, 'HH24:MI') AS end_time,
                  created_at, updated_at
           FROM schedules
           WHERE (
             (:nid > 0 AND nurse_id = :nid) OR
             (:email <> '' AND lower(COALESCE(nurse_email,'')) = :email) OR
             (:name <> '' AND nurse = :name)
           )
   ...
   foreach ($stmt as $row) {
     ...
     $requests[] = [
       'id'           => (int)($row['id'] ?? 0),
       'nurse'        => (string)($row['nurse'] ?? $nurseName),
       'nurse_id'     => (int)($row['nurse_id'] ?? $nurseId),
       'nurse_email'  => (string)($row['nurse_email'] ?? $nurseEmail),
...
<div class="layout-sidebar full-bleed" style="padding: 24px 20px;">
  ...
  <div class="appt-layout">
    ...
    <aside class="appt-list" style="margin-left:12px;">
      ...
      <div id="shiftList" style="display:flex;flex-direction:column;gap:10px;"></div>
  ...
  <script>
    (function(){
      ...
      function renderRequests(items){
        ...
        sorted.forEach(function(it){ appendClientRequest(it, true); });
      }
      ...
      async function loadRequests(){
        ...
        // Show immediate placeholder to avoid delay before empty-state appears
        ...
        try{
          ...
          // Show nurse's own schedules including pending/request, plus accepted/created
          var allowed = ['accepted','created','pending','request'];
          var visible = merged.filter(function(it){
            var s = String(it.status||'').toLowerCase();
            return allowed.indexOf(s) !== -1;
          });
          console.debug('[nurse_schedule] mine:', mine, 'visible:', visible);
